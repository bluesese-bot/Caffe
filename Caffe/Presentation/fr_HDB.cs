...
namespace Caffe.Presentation
{
    public partial class fr_HDB : Form
    {
        ...
        public void khoitaoluoi()
        {
            ...
            msds.Columns[3].HeaderText = "Bàn";
            msds.Columns[3].Width = 50;
            msds.Columns[4].HeaderText = "Tổng Tiền";
            msds.Columns[4].Width = 100;
            msds.Columns[5].HeaderText = "Trang Thái";
            msds.Columns[5].Width = 100;
        }
        public void hienthi()
        {
            string sql = "SELECT     mahdb, manv,ngayban, maBan,tongtien,trangThai FROM tb_HDB";
        ...
        private void btmoi_Click(object sender, EventArgs e)
        {
            ...
            setnull();
            txtma.Enabled = true;
            thucthi.loadhdAuto(txtma);
            txtma.Text = (int.Parse(txtma.Text) + 1).ToString();
        }
        ...
        private void msds_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            lbnv.Text = msds.Rows[dong].Cells[1].Value.ToString();
            lbncc.Text = msds.Rows[dong].Cells[3].Value.ToString();
            txttt.Text = msds.Rows[dong].Cells[4].Value.ToString();
            thucthi.loadtennv(cbnv,lbnv.Text);
            thucthi.loadtenkh(cbncc,lbncc.Text);
            locktext();
        }

        private void fr_HDB_Load(object sender, EventArgs e)
        {
            thucthi.loadAllban(cbncc);
            thucthi.loadAllnv(cbnv);
            hienthi();
            khoitaoluoi();
            locktext();
        }
        ...
        {
            lbnv.Text = thucthi.loadmanv(lbnv.Text, cbnv.Text);
        ...
        {
            lbncc.Text = thucthi.loadmakh(lbncc.Text, cbncc.Text);
